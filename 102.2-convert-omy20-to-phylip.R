...
# This version creates a phylip of only omy20 genotypes
# ./102.2-convert-omy20-to-phylip.R path-to-file file bamlist
...
omy20<-genos %>% filter(X1 == "omy20" &  X2 >= 5446054  & X2 <= 18985808)


seq<-select(omy20, -1:-2)
...
write(cbind(inds, characters)[1:2], file=paste(dir,basename,"-omy20",".phy",sep="")) #put header in
write.table(trans, file=paste(dir,basename,"-omy20",".phy",sep=""), append=TRUE, quote = FALSE, sep="", row.names = TRUE, col.names=FALSE)
