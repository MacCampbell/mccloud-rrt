...
#Make a genind
gdf<-as.loci(data[1:1000], sep="/", ploidy=2)
...
df2<-as_tibble(df) %>% group_by(population) %>% mutate(Sum=sum(count)) %>% select(population, Sum) %>% unique() %>% ungroup() %>% mutate(Total=sum(Sum))
