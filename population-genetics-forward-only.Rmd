...
write_tsv(bams, "bamlists/388.single.bamlist", col_names = FALSE)
...
```{sh, eval=FALSE}
srun -p high -t 48:00:00 --mem=32G --nodes=1 $HOME/angsd/angsd -P 12  -bam bamlists/388.single.bamlist -minInd 349 -GL 1 -ref genome/omyV6Chr.fasta \
-doGLF 2 -doMajorMinor 1 -doMaf 2 -SNP_pval 1e-6 -minMapQ 20 -minQ 20 -r omy01: \
-out outputs/population-genetics-forward/383.single > outputs/population-genetics-forward/beagle.out 2> outputs/population-genetics-forward/beagle.err &

srun -p high -t 1:00:00 python $HOME/pcangsd/pcangsd.py -beagle outputs/population-genetics-forward/388.single.beagle.gz -o outputs/population-genetics-forward/388.single -threads 10
```
...
ggsave("outputs/population-genetics-single/259.PC12.maf.redb.jpeg")
```
...
write_tsv(select(crt, Singles), "bamlists/242.bamlist",col_names = FALSE)

```

```{sh, eval=FALSE}
srun -p high -t 48:00:00 --mem=32G --nodes=1 $HOME/angsd/angsd -P 12  -bam bamlists/242.bamlist -minInd 230 -GL 1 -ref genome/omyV6Chr.fasta \
-doGLF 2 -doMajorMinor 1 -doMaf 2 -SNP_pval 1e-6 -minMapQ 20 -minQ 20 -r omy01: \
-out outputs/population-genetics-forward/242.single > outputs/population-genetics-forward/242beagle.out 2> outputs/population-genetics-forward/242beagle.err &


srun -p high -t 1:00:00 python $HOME/pcangsd/pcangsd.py -beagle outputs/population-genetics-forward/242.single.beagle.gz -o outputs/population-genetics-forward/242.single -threads 10

srun -p high -t 1:00:00 python $HOME/pcangsd/pcangsd.py -beagle outputs/population-genetics-forward/242.single.beagle.gz -o outputs/population-genetics-forward/242.single.3 -minMaf 0.3 -threads 10
```

Number of sites after MAF filtering (0.05): 1589
Number of sites after MAF filtering (0.3): 409


```{sh, eval=FALSE}
srun -p high -t 48:00:00 --mem=32G --nodes=1 $HOME/angsd/angsd -P 12 -bam bamlists/242.bamlist -minInd 218 -GL 1 -doPlink 2 -doMajorMinor 1 -doMaf 1 \
-SNP_pval 1e-6 -minMapQ 20 -minQ 20 -minMaf 0.05 -doGeno 4 -doPost 1 -postCutoff 0.95 -r omy01: -out outputs/population-genetics-forward/242.plink  > outputs/population-genetics-forward/242plink.out 2> outputs/population-genetics-forward/242plink.err &
```

```{r, warning=FALSE, message=FALSE}
cov<-read_delim("outputs/population-genetics-single/242.single.cov", col_names=FALSE, delim=" ") %>% as.matrix()
```
