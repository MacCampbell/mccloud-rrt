...
Number of sites after MAF filtering (0.05): 1915 for omy01
...
cov<-read_delim("outputs/population-genetics-single/339.single.cov", col_names=FALSE, delim=" ") %>% as.matrix()
```
...
ggplot(sub12 %>% filter(MajorGroup %in% c("REDB","MCRT")), aes(x = val_x, y = val_y, fill = Watershed, shape=MajorGroup)) +
  geom_point(size = 2, alpha=0.75) +
  scale_fill_discrete(na.value = "white") + 
  theme_bw()+
  theme(panel.grid=element_blank())+
