...
cov<-read_delim("outputs/population-genetics-single/388.single.cov", col_names=FALSE, delim=" ") %>% as.matrix()
```
...
ggsave("outputs/population-genetics-single/388.pcs123.jpeg")
```
...
ggplot(sub13 %>% filter(MajorGroup %in% c("REDB","CRT")), aes(x = val_x, y = val_y, fill = Group, shape=MajorGroup)) +
  geom_point(size = 2, alpha=0.75) +
  scale_fill_discrete(na.value = "white") + 
  theme_bw()+
  theme(panel.grid=element_blank())+
...
```{sh, eval=FALSE}
srun -p high -t 48:00:00 --mem=32G --nodes=1 $HOME/angsd/angsd -P 12  -bam bamlists/286.bamlist -minInd 218 -GL 1 -ref genome/omyV6Chr.fasta \
-doGLF 2 -doMajorMinor 1 -doMaf 2 -SNP_pval 1e-6 -minMapQ 20 -minQ 20 -r omy01: \
...
srun -p high -t 1:00:00 python $HOME/pcangsd/pcangsd.py -beagle outputs/population-genetics-forward/286.single.beagle.gz -o outputs/population-genetics-forward/242.single -threads 10

srun -p high -t 1:00:00 python $HOME/pcangsd/pcangsd.py -beagle outputs/population-genetics-forward/286.single.beagle.gz -o outputs/population-genetics-forward/242.single.3 -minMaf 0.3 -threads 10
```
...
-SNP_pval 1e-6 -minMapQ 20 -minQ 20 -minMaf 0.05 -doGeno 4 -doPost 1 -postCutoff 0.95 -out outputs/population-genetics-forward/242.plink  > outputs/population-genetics-forward/242plink.out 2> outputs/population-genetics-forward/242plink.err &
```
