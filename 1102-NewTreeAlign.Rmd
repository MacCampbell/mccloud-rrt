...
cov<-read_delim("outputs/1102/299.cov", col_names=FALSE, delim=" ") %>% as.matrix()
...
python $HOME/pcangsd/pcangsd.py -beagle outputs/1102/299.beagle.gz -kinship -admix -admix_K 4 -o outputs/1102/admix4 -threads 10
python $HOME/pcangsd/pcangsd.py -beagle outputs/1102/299.beagle.gz -kinship -admix -admix_K 5 -o outputs/1102/admix5 -threads 10
 
python $HOME/pcangsd/pcangsd.py -beagle outputs/1102/299.beagle.gz -kinship -admix -admix_K 6 -o outputs/1102/admix6 -threads 10
...
```{r}
reds<- k %>% filter(Group=="REDB") %>% arrange(Watershed) %>% mutate(Index=1:n()) %>%
...
#  geom_segment(x=0, xend= pops2$Stop[length(pops2$Stop)], y=1, yend=1, alpha=0.9, size=0.25) +
  ...
  scale_x_continuous(breaks=pops2$Position, labels=pops2$Group) +
  xlab("") +
  facet_grid(.~Watershed, scales="free_x", space="free")

ggsave("outputs/1102/redsk6.jpeg", width=10)
```
